...
function setupRound(room, params, seonIdx, dealt, pickReveals) {
  ...
  room.gs.round = {
    ...
    bbeokCount: {}, bbeokBy: {}, goCount: room.queue.map(() => 0), goScoreAt: room.queue.map(() => 0),
...
// 뻑 스택 회수 시 뺏을 피: 자뻑(내가 만든 뻑을 내가 회수)이면 2장
function bbeokSteal(room, r, cfg, seat, m) {
  const self = cfg.hyojaBbeok && r.bbeokBy[m] === seat;
  delete r.bbeokBy[m];
  stealPi(room, seat, self ? 2 : 1, self ? '자뻑' : '뻑먹기');
}
...
function resolveTurn(room, seat, d) {
  ...
  if (d && M !== 0 && d.m === M) {
    ...
    if (cB === 0) {                               // 쪽: 미리 없던 월에 깐 패를 바로 먹음
      ...
      if (cB === 2) stealPi(room, seat, 1, '따닥'); else bbeokSteal(room, r, cfg, seat, M);   // cB≥3=뻑 스택 회수(자뻑이면 2장)
      captured = true;
    }
  } else {
    ...
    if (M !== 0) {
      if (cB === 1) {                              // 짝 먹기(손패 + 바닥1)
        ...
        bbeokSteal(room, r, cfg, seat, M); captured = true;   // 뻑 스택 회수(자뻑이면 2장)
      } // cB===0: 그냥 깔림(이미 바닥)
    }
...
function resolveFlip(room, seat, d) {
  ...
  bbeokSteal(room, r, cfg, seat, N); return true;   // 뻑 스택 회수(자뻑이면 2장)
}
...
function declareBomb(room, seat, month) {
  ...
  const fc = floorCount(r, month);
  // 폭탄: 손3+바닥1(이상) 또는 두장 폭탄(손2+바닥2)
  if (!((handM.length >= 3 && fc >= 1) || (handM.length >= 2 && fc >= 2))) return false;
...
module.exports = {
  ...
  action(room, ws, msg) {
    ...
    if (msg.type === 'shake') return declareShake(room, seat, Number(msg.month != null ? msg.month : msg.m));  // 흔들기(클라 m / 봇 month 둘 다 허용)
    if (msg.type === 'bomb') return declareBomb(room, seat, Number(msg.month != null ? msg.month : msg.m));    // 폭탄
  ...
  state(room, ws) {
    ...
    return {
      ...
      bombable: myTurnNow(r, seatIdx) ? bombMonths(r, seatIdx) : [],  // 폭탄 가능 월(손3+바닥1 / 손2+바닥2)
