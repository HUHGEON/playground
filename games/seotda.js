...
const REVEAL_MS    = Number(process.env.SEOTDA_REVEAL_MS) || 2000;  // 재경기(구사·멍구사·동점) 전 패 공개 시간
const MAX_SEATS   = 5;
...
function showdown(room) {
  ...
  if (redealers.length) {
    // 구사/멍구사는 무조건 재경기 — 먼저 패를 공개(상대도 사유 확인)한 뒤 자동 재경기
    h.redealers = redealers; h.redealPassed = new Set();
    const rset = new Set(redealers);
    const reveals = h.reveals.map((r, i) => ({ ...r, redeal: rset.has(h.entries[i].ws) }));
    room.ctx.notify(room, `${redealers.map((w) => w.name).join(', ')}님 구사/멍구사 — 무조건 재경기!`);
    pauseThenRedeal(room, {
      redeal: true, reason: '구사·멍구사', redealers: redealers.map((w) => w.name), reveals, pot: h.pot,
    }, () => executeRedeal(room));
    return;
  }
...
function finalizeShowdown(room, contenders) {
  ...
  if (contenders.length > 1 && winners.length > 1) {
    // 동점 → 분배 X. 먼저 패 공개(상대도 동점 확인) 후 판돈 묻고 재경기
    pauseThenRedeal(room, {
      redeal: true, tie: true, reason: '동점', winners: winners.map((w) => ({ name: w.name, color: w.color })), reveals, pot: totalPot,
    }, () => {
...
// 재경기(구사·멍구사·동점) 전 패 공개 페이즈 — 상대도 사유를 보게 잠시 멈춘 뒤 redealFn 실행
function pauseThenRedeal(room, resultPayload, redealFn) {
  const gs = room.gs, h = gs.hand;
  clearActionTimer(h); clearStage(gs);
  h.result = resultPayload;                     // 패 공개 + 재경기 사유 배너(아직 carry/토스트 X)
  room.phase = 'finished';
  gs.revealDeadline = Date.now() + REVEAL_MS;
  gs._redealFn = redealFn;                       // (테스트 flush용)
  if (gs.revealTimer) clearTimeout(gs.revealTimer);
  gs.revealTimer = setTimeout(() => {
    gs.revealTimer = null; gs.revealDeadline = null; gs._redealFn = null;
    redealFn();                                 // carry + "묻고 더블로 가!" 토스트 + 합류 단계
    room.ctx.broadcastRoom(room); room.ctx.broadcastLobby();
  }, REVEAL_MS);
}
...
module.exports = {
  ...
  canStart(room) {
    ...
    if (room.gs.revealTimer) return false;                                          // 재경기 패 공개 중엔 시작 불가
  ...
  cleanup(room) { clearActionTimer(room.gs.hand); clearStage(room.gs); clearAutoStart(room.gs); if (room.gs.revealTimer) clearTimeout(room.gs.revealTimer); },
  ...
  state(room, ws) {
    ...
    // 단계별 안내/버튼
    const stageDeadline = gs.revealTimer ? gs.revealDeadline
      : (room.phase === 'redeal' || room.phase === 'rejoin') ? gs.stageDeadline : (h && h.deadline);
    ...
    return {
      ...
      revealing: !!gs.revealTimer,                       // 재경기 전 패 공개 중
  ...
  // 테스트용: 재경기 패 공개 대기(setTimeout)를 즉시 진행
  _flushReveal(room) {
    const gs = room.gs;
    if (gs.revealTimer) { clearTimeout(gs.revealTimer); gs.revealTimer = null; gs.revealDeadline = null; }
    if (gs._redealFn) { const fn = gs._redealFn; gs._redealFn = null; fn(); }
  },
};
